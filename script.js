...
function toTensorFromBox(box, size = 160) {
  ...
  if (MIRROR_UI) {
    box.x = cw - (box.x + box.width);
  }

  const sx = clamp(Math.round(box.x * (vw / cw)), 0, vw);
  const sy = clamp(Math.round(box.y * (vh / ch)), 0, vh);
  const sw = clamp(Math.round(box.width  * (vw / cw)), 1, vw - sx);
  const sh = clamp(Math.round(box.height * (vh / ch)), 1, vh - sy);
...
async function runOnnxEmotion(which) {
  ...
  const tensor = trackBox ? toTensorFromBox(trackBox, 160) : toTensorFromBox({x:0,y:0,width:videoEl.clientWidth,height:videoEl.clientHeight}, 160);
