...
// ✅ Pure function: does NOT mutate the incoming box
function toTensorFromBox(box, size = 160) {
  ...
  // compute mirrored coordinates WITHOUT changing the original
...
async function runOnnxEmotion(which) {
  ...
  const tensor = trackBox
    ? toTensorFromBox(trackBox, 160)
    : toTensorFromBox({x:0,y:0,width:videoEl.clientWidth,height:videoEl.clientHeight}, 160);
